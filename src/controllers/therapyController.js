...
const getSingleTherapist = async (req, res) => {
  try {
    ...
    if (!mongoose.isValidObjectId(therapistID)) {
      return res
        .status(400)
        .json({ message: "Invalid Therapist ID provided." });
    }
    ...
    if (!therapist) {
      return res
        .status(404)
        .json({ message: `Therapist with id ${therapistID} not found.` });
    }
...
const getSingleAppointment = async (req, res) => {
  try {
    ...
    if (!mongoose.isValidObjectId(appointmentID)) {
      return res
        .status(400)
        .json({ message: "Invalid Appointment ID provided." });
    }
    ...
    if (!appointment) {
      return res
        .status(404)
        .json({ message: `Appointment with id ${appointmentID} not found.` });
    }
